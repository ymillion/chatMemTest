...
def update_seed_display(seed_status, seed_message):
    if seed_status == "forgotten":
        return gr.update(value=seed_message, label="⚠️ Seed Status ⚠️", interactive=False)
    elif seed_status == "set":
        return gr.update(value=seed_message, label="Seed Status", interactive=False)
    else:  # "remembered"
        return gr.update(value=seed_message, label="Seed Status", interactive=False)
...
def run_batch_process(chatbot, status, stop_threshold):
    try:
        ...
        yield chatbot, benchmark.get_retention_score(), plot_retention_history(), status, benchmark.get_token_metrics(), update_seed_display(*benchmark.get_seed_metrics())
...
with gr.Blocks() as demo:
    ...
    with gr.Row():
        ...
        with gr.Column(scale=1):
            ...
            seed_display = gr.Textbox(label="Seed Status", interactive=False)
    ...
    def respond(message, chat_history):
        ...
        seed_status, seed_message = benchmark.get_seed_metrics()
        return "", chat_history, benchmark.get_retention_score(), plot_retention_history(), benchmark.get_token_metrics(), update_seed_display(seed_status, seed_message)

    def reset_all():
        result = benchmark.reset()
        seed_status, seed_message = benchmark.get_seed_metrics()
        return [], result, benchmark.get_retention_score(), None, benchmark.get_token_metrics(), "Balanced", 0.7, 1.0, 20000, "Ready to start batch processing.", update_seed_display(seed_status, seed_message)

    def set_seed(seed):
        benchmark.set_seed(seed)
        seed_status, seed_message = benchmark.get_seed_metrics()
        return update_seed_display(seed_status, seed_message)
    ...
    msg.submit(respond, [msg, chatbot], [msg, chatbot, retention_score, retention_plot, token_metrics, seed_display])
    clear.click(reset_all, outputs=[chatbot, msg, retention_score, retention_plot, token_metrics, personality, temperature, top_p, max_tokens, status, seed_display])
    set_seed_button.click(set_seed, inputs=[seed_input], outputs=[seed_display])
    ...
    for button in preset_buttons:
        button.click(lambda x: x, button, msg).then(
            respond, [msg, chatbot], [msg, chatbot, retention_score, retention_plot, token_metrics, seed_display]
        )

    batch_button.click(run_batch_process, [chatbot, status, stop_threshold], [chatbot, retention_score, retention_plot, status, token_metrics, seed_display])
